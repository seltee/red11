fxc.exe .\UVSimpleVertexShader.hlsl /Fo /Tvs_3_0
fxc.exe .\UVSimpleFragmentShader.hlsl /Fo /Tps_3_0
fxc.exe .\UVSimpleMaskVertexShader.hlsl /Fo /Tvs_3_0
fxc.exe .\UVSimpleMaskFragmentShader.hlsl /Fo /Tps_3_0
fxc.exe .\UVVertexShader.hlsl /Fo /Tvs_3_0
fxc.exe .\UVFragmentShader.hlsl /Fo /Tps_3_0
fxc.exe .\UVNormalVertexShader.hlsl /Fo /Tvs_3_0
fxc.exe .\UVNormalFragmentShader.hlsl /Fo /Tps_3_0
fxc.exe .\UVSkinnedVertexShader.hlsl /Fo /Tvs_3_0
fxc.exe .\UVSkinnedFragmentShader.hlsl /Fo /Tps_3_0
fxc.exe .\UVSkinnedNormalVertexShader.hlsl /Fo /Tvs_3_0
fxc.exe .\UVSkinnedNormalFragmentShader.hlsl /Fo /Tps_3_0
fxc.exe .\UVShadowVertexShader.hlsl /Fo /Tvs_3_0
fxc.exe .\UVShadowFragmentShader.hlsl /Fo /Tps_3_0
fxc.exe .\UVShadowMaskVertexShader.hlsl /Fo /Tvs_3_0
fxc.exe .\UVShadowMaskFragmentShader.hlsl /Fo /Tps_3_0
fxc.exe .\UVShadowSkinnedVertexShader.hlsl /Fo /Tvs_3_0
bin2header.exe .\UVSimpleVertexShader.vso
bin2header.exe .\UVSimpleFragmentShader.pso
bin2header.exe .\UVSimpleMaskVertexShader.vso
bin2header.exe .\UVSimpleMaskFragmentShader.pso
bin2header.exe .\UVVertexShader.vso
bin2header.exe .\UVFragmentShader.pso
bin2header.exe .\UVNormalVertexShader.vso
bin2header.exe .\UVNormalFragmentShader.pso
bin2header.exe .\UVSkinnedVertexShader.vso
bin2header.exe .\UVSkinnedFragmentShader.pso
bin2header.exe .\UVSkinnedNormalVertexShader.vso
bin2header.exe .\UVSkinnedNormalFragmentShader.pso
bin2header.exe .\UVShadowVertexShader.vso
bin2header.exe .\UVShadowFragmentShader.pso
bin2header.exe .\UVShadowMaskVertexShader.vso
bin2header.exe .\UVShadowMaskFragmentShader.pso
bin2header.exe .\UVShadowSkinnedVertexShader.vso