...
bool LoaderFBX::loadFBXFile(std::string path, std::vector<MeshObject *> *meshObjectsList, std::vector<Animation *> *animationsList)
{
    ...
    for (auto &it : objects->children)
    {
        if (it->isName("Geometry"))
        {
            ...
            if (descriptor.getPolygons() && descriptor.getVertexes())
            {
                Mesh *mesh = new Mesh(VertexDataType::PositionUV, descriptor.getVertexes(), descriptor.getVertexAmount(), descriptor.getPolygons(), descriptor.getPolygonsAmount());
    ...
    for (auto &it : connections->children)
    {
        ...
        FBXModelWithIndex *foundModel = getModelByIndex(indexFrom, models);
        if (foundModel)
        {
            if (indexTo)
        ...
        FBXMeshWithIndex *foundMesh = getMeshByIndex(indexFrom, meshes);
        if (foundMesh)
        {
            foundModel = getModelByIndex(indexTo, models);
            if (foundModel)
            {
                foundModel->meshObject = new MeshObject(foundMesh->mesh);
                foundModel->meshObject->setPosition(foundModel->position);
                foundModel->meshObject->setRotation(foundModel->rotation);
                foundModel->meshObject->setScale(foundModel->scale);

                meshObjectsList->push_back(foundModel->meshObject);
            }
            continue;
        }

        FBXAnimationLayer *layer = getLayerByIndex(indexFrom, animLayers);
        if (layer)
        {
            FBXAnimationStack *stack = getStackByIndex(indexTo, animStacks);
            if (stack)
            {
        ...
        FBXAnimationCurveNode *curveNode = getCurveNodeByIndex(indexFrom, animCurveNodes);
        if (curveNode)
        {
            FBXModelWithIndex *targetModel = getModelByIndex(indexTo, models);
            FBXAnimationLayer *targetLayer = getLayerByIndex(indexTo, animLayers);
            if (targetModel)
            {
        ...
        FBXAnimationCurve *curve = getCurveByIndex(indexFrom, animCurves);
        if (curve)
        {
            FBXAnimationCurveNode *curveNode = getCurveNodeByIndex(indexTo, animCurveNodes);
            if (curveNode)
            {
    ...
    for (auto &model : models)
    {
        ...
        {
            auto parentModel = getModelByIndex(model.parentIndex, models);
            if (parentModel && model.meshObject && parentModel->meshObject)
            {
    ...
    {
        if (model.meshObject)
            model.meshObject->setName(model.name);
    }
...
FBXModelWithIndex *LoaderFBX::getModelByIndex(unsigned long long id, std::vector<FBXModelWithIndex> &models)
{
    for (auto &it : models)
    {
        if (it.index == id)
...
FBXMeshWithIndex *LoaderFBX::getMeshByIndex(unsigned long long id, std::vector<FBXMeshWithIndex> &meshes)
{
    for (auto &it : meshes)
    {
        if (it.index == id)
...
FBXAnimationStack *LoaderFBX::getStackByIndex(unsigned long long id, std::vector<FBXAnimationStack *> &animStacks)
{
...
FBXAnimationLayer *LoaderFBX::getLayerByIndex(unsigned long long id, std::vector<FBXAnimationLayer *> &animLayers)
{
...
FBXAnimationCurveNode *LoaderFBX::getCurveNodeByIndex(unsigned long long id, std::vector<FBXAnimationCurveNode *> &animCurveNodes)
{
...
FBXAnimationCurve *LoaderFBX::getCurveByIndex(unsigned long long id, std::vector<FBXAnimationCurve *> &animCurves)
{
